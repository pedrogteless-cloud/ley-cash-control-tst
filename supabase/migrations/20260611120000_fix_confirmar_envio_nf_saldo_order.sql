-- Fix: when creating an auto caixa entry (no entry for today yet), pick the
-- most recent entry by movement date (DD/MM), not by creation timestamp.
-- Using created_at DESC would silently pick a retroactively-added past entry
-- over a legitimately newer entry.
...
BEGIN
  ...
  ELSE
    -- Order by movement date (month then day), not by creation timestamp,
    -- so a retroactively-added past entry never shadows a newer dated entry.
    SELECT saldo_total INTO v_last_saldo
      FROM public.caixa_movimentos
      ORDER BY
        split_part(data, '/', 2)::int DESC,
        split_part(data, '/', 1)::int DESC,
        created_at DESC
      LIMIT 1;
