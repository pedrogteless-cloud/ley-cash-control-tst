-- Harden devolvido updates:
-- - validate values server-side
-- - lock the row while calculating recovery deltas
-- - notify Telegram with recovered deltas, not final field totals
...
BEGIN
  ...
  IF p_valor_devolvido IS NULL
    OR p_valor_rec_fornecedor IS NULL
    OR p_valor_rec_empresa IS NULL
    OR p_valor_devolvido < 0
    OR p_valor_rec_fornecedor < 0
    OR p_valor_rec_empresa < 0
  THEN
    RAISE EXCEPTION 'invalid_money_values';
  END IF;

  IF p_valor_rec_fornecedor + p_valor_rec_empresa > p_valor_devolvido THEN
    RAISE EXCEPTION 'recovered_exceeds_devolvido';
  END IF;
  ...
  v_pendente_lancamento := p_valor_devolvido - v_total_recuperado_lancamento;

  UPDATE public.cheques_devolvidos
     SET data = p_data::date,
  ...
  IF v_rec_forn_delta > 0 OR v_rec_emp_delta > 0 THEN
    ...
    v_payload := jsonb_build_object(
      ...
      'data', p_data,
      'rec_fornecedor_delta', CASE WHEN v_rec_forn_delta > 0 THEN v_rec_forn_delta ELSE NULL END,
      'rec_empresa_delta', CASE WHEN v_rec_emp_delta > 0 THEN v_rec_emp_delta ELSE NULL END,
      'total_recuperado_delta',
        GREATEST(v_rec_forn_delta, 0) + GREATEST(v_rec_emp_delta, 0),
